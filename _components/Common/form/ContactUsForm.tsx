...
export default function ContactUsForm() {
    ...
    const handleSubmit = async (values: typeof initialValues) => {
        ...
        try {
            const res = await fetch(`https://apolloniblom.awareness-profiling.com/api/forms/contact`, {
                method: "POST",
        ...
        <div className="">
            {!isSubmitted ? (
                ...
                >
                    {({ setFieldValue, values, errors, touched }) => (
                        <Form className="flex flex-col md:gap-4 gap-3  text-primary ">
                            ...
                            <button
                                type="submit"
                                className={`flex relative bg-black text-white hover:text-black hover:bg-white transition-all duration-500 overflow-hidden gap-3 w-full justify-center text-base font-medium rounded-full py-3 ${isLoading ? " pointer-events-none opacity-40" : ""}`}
                            >
                ...
                <div className=" h-[400px] text-center content-center">
                    <h3 className={` text-primary text-3xl my-5`}>{t("thanks.lead.title")}</h3>
